  ...
  useEffect(() => {
    const fetchRideAndUsers = async () => {
      try {
        ...
        // Check if user should review this ride
              ...
              <div className="flex gap-3">
                {(ride.status === "SCHEDULED" || ride.status == null) && ride.acceptedPassengers && ride.acceptedPassengers.length > 0 && (
                  <button
                    ...
                    className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-500 text-white rounded-lg font-medium transition flex items-center gap-2"
                  >
                    {actionLoading ? (
                ...
                {ride.status === "IN_PROGRESS" && (
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-4 py-2 bg-red-600 hover:bg-red-700 disabled:bg-gray-500 text-white rounded-lg font-medium transition flex items-center gap-2"
                  >
                    {actionLoading ? (
                      <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                    ) : (
