...
export default function Dashboard() {
  ...
  const enrichRidesWithUserDetails = async (rides) => {
    ...
    // Fetch all unique user details in parallel
    if (userIdsToFetch.size > 0) {
      const userPromises = Array.from(userIdsToFetch).map(async (userId) => {
        try {
          ...
          userCache.set(userId, { id: userId, name: userRes.data.name, email: userRes.data.email });
        } catch (err) {
          // Cache unknown user details
          userCache.set(userId, { id: userId, name: "Unknown User", email: "" });
        }
      });
  ...
  const checkReviewedRides = async (completedRides) => {
    try {
      const reviewedRideIds = new Set();

      // Check reviews for each completed ride in parallel
      const reviewPromises = completedRides.map(async (ride) => {
        try {
          ...
          return userReview ? ride.id : null;
        } catch (err) {
          // If review endpoint fails, continue checking other rides
          console.error(`Failed to check reviews for ride ${ride.id}:`, err);
          return null;
        }
      });

      const results = await Promise.all(reviewPromises);
      results.forEach(rideId => {
        if (rideId) reviewedRideIds.add(rideId);
      });

      setReviewedRides(reviewedRideIds);
    } catch (err) {
  ...
  const fetchRidesAndUsers = async () => {
    ...
    try {
      ...
      if (user.role === "OWNER") {
        ...
        // Split scheduled rides into upcoming and past
        upcomingRides = scheduledRides.filter(r => new Date(r.startTime) > now)
          .sort((a, b) => new Date(a.startTime) - new Date(b.startTime)); // Sort by start time ascending
        
        const pastScheduledRides = scheduledRides.filter(r => new Date(r.startTime) <= now);
        
        // Combine past scheduled rides with completed rides
        pastRides = [...pastScheduledRides, ...completed].sort((a, b) => new Date(b.startTime) - new Date(a.startTime)); // Sort by start time descending (most recent first)
      ...
      if (completed.length > 0) {
        await checkReviewedRides(completed);
      }
      ...
      >
        ...
        <div className="mb-4">
          <h2 className="text-xl font-bold text-gray-800 pr-20 flex items-center gap-2">
            ...
            {/* Status Label for Created Rides */}
            {isOwnerView && (
        ...
        {ride.status === "SCHEDULED" && ride.active && new Date(ride.startTime) < new Date() && (
          <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center gap-3">
            ...
            <div className="text-xs text-red-700">
              This ride was scheduled for {new Date(ride.startTime).toLocaleString()} but was never started
            </div>
          </div>
        )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      {user.role === "OWNER" && <>
        ...
        {renderSection("🚀 Rides in Progress", inProgressRides, user.role === "OWNER")}
        
        {/* Created Rides as Owner - Main Section */}
        <h2 className="text-2xl font-bold mt-8 mb-6 flex items-center gap-2 text-gray-800">
          🚗 Created Rides as Owner
        </h2>
        <div className="space-y-6">
          {renderSection("Upcoming Rides", upcomingRides, true)}
          {renderSection("Past Rides", pastRides, true)}
        </div>
