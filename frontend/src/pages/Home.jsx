...
export default function Home() {
  ...
  const fetchRides = async () => {
    ...
    try {
      ...
      let url;
      const hasFilters = origin.trim() || destination.trim();
      ...
      const activeRides = (res.data || []).filter(ride => ride.active !== false);
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        {loading ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-2xl font-semibold text-gray-800 mb-2">Loading Rides...</h3>
            <p className="text-lg text-gray-600">Please wait while we fetch available rides for you</p>
          </div>
        ) : error ? (
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            alert("Ride request sent successfully! Check your dashboard for status.");
                            fetchRides(); // Refresh rides
                          } catch (err) {
                            console.error(err);
                            alert(err?.response?.data?.message || "Failed to send request. You may have already requested this ride.");
                          }
                        }}
